                                    ...
                                    <tr>
                                                ...
                                                <div style="min-width: 0;"> {{-- Prevent flex overflow --}}
                                                    ...
                                                    <div class="d-flex flex-wrap gap-1">
                                                        ...
                                                        @foreach($member->divisions as $div)
                                                            ...
                                                            @if($loop->iteration >= 2)
                                                                <span class="badge bg-light text-muted border" style="font-size: 10px;">+{{ $member->divisions->count() - 2 }}</span>
                                                                @break
                                                            @endif
                                                        @endforeach
                                                    </div>
                                        ...
                                        <td>
                                            ...
                                            @if ($attendance)
                                                @if ($attendance->check_out_time)
                                                    <span class="badge bg-primary bg-opacity-10 text-primary mb-1 rounded-pill px-3">
                                                        <i class="mdi mdi-home-variant me-1"></i>Pulang: {{ $attendance->check_out_time->format('H:i') }}
                                                    </span>
                                                @else
                                                    <span class="badge bg-success bg-opacity-10 text-success mb-1 rounded-pill px-3">
                                                        <i class="mdi mdi-briefcase-check me-1"></i>Masuk: {{ $attendance->check_in_time->format('H:i') }}
                                                    </span>
                                                @endif
                                            @elseif ($member->activeLateStatus)
                                                <span class="badge bg-warning bg-opacity-10 text-warning mb-1 rounded-pill px-3">
                                        ...
                                        <td>
                                            @if ($attendance)
                                                ...
                                                @endphp
                                                @if($photo)
                                                    <button type="button" class="btn btn-sm btn-outline-secondary image-popup d-flex align-items-center gap-2" 
                                                            data-bs-toggle="modal" data-bs-target="#imageModal" 
                                                            data-src="{{ Storage::url($photo) }}">
                                                        <div style="width: 30px; height: 30px; overflow: hidden;" class="rounded">
                                                ...
                                                @endif
                                            @elseif ($member->activeLateStatus)
                                                <div class="small text-muted fst-italic text-truncate border-start border-3 border-warning ps-2" style="max-width: 200px;">
